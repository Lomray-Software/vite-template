...
| `example/cloudflare` | The minimal app deployed to Cloudflare Workers: a Worker entry built with `ssr-boost build --focus-only all`, Workers Static Assets, a KV binding read in a loader, `wrangler dev` preview, smoke and browser checks | Deploy to Cloudflare Workers | [Browse](https://github.com/Lomray-Software/vite-template/tree/example/cloudflare) |
